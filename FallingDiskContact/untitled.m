...
Eref = 1/2*m*(Xdot.^2 + Ydot.^2 + R^2*sin(theta).*thetadot.^2) + ...
    1/2*(I_A*(psidot - phidot.*sin(theta)).^2 + ...
        I_T*(thetadot.^2 + phidot.^2.*(cos(theta).^2))) + ...
    m*g*R*cos(theta);
clearvars -except Econtacto Eode15i Eref t0 tf
h = 0.1;
t = t0:h:tf;
...
plot(th10, Eref, '-k')
xlim([t0 tf])
legend({'Contacto (orden 2)','ode15i','Referencia'}, 'Interpreter','latex')
xlabel('Tiempo (s)')
