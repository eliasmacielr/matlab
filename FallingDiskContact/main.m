...
eq_phi = I_A*(((psi_l-psi_k)/h - (phi_l-phi_k)/h*sin(theta_k))*sin(theta_k)/h - ((psi_k-psi_j)/h - (phi_k-phi_j)/h*sin(theta_j))*sin(theta_j)/h) ...
    + I_T*((cos(theta_k))^2*(phi_l-phi_k)/h^2 + (cos(theta_j))^2/h) - lambda_1*R*sin(theta_k)*cos(phi_k) - lambda_2*R*sin(theta_k)*sin(phi_k);
...
% Constraints
eq_X = ((X_l-X_k)/h) + R*cos(theta_k)*sin(phi_k)*(theta_l-theta_k)/h + R*sin(theta_k)*cos(phi_k)*(phi_l-phi_k)/h - R*cos(phi_k)*(psi_l-psi_k)/h;
eq_Y = ((Y_l-X_k)/h) - R*cos(theta_k)*cos(phi_k)*(theta_l-theta_k)/h + R*sin(theta_k)*sin(phi_k)*(phi_l-phi_k)/h - R*sin(phi_k)*(psi_l-psi_k)/h;
